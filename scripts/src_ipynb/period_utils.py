# period_utils.py
"""
Generation of biweekly periods (1-15, 16-end of month), shared
between temperature_profile.py and precipitation_profile.py, so that both
series use exactly the same periodo_inicio/periodo_fin (period_start/period_end)
and can be directly joined by that column.
"""
...
def build_biweekly_periods(start_date, end_date):
    ...
    while current <= end_date:
        ...
        # Only include Q1 if its end date falls within the allowed range
        # timedelta(days=1) allows the period to be considered complete
        # even on the exact end date (inclusive check)
        if q1_end <= end_date + timedelta(days=1):
            periods.append((f'{year}-{month:02d}_Q1', q1_start, q1_end))
        
        # Only include Q2 if its end date falls within the allowed range
